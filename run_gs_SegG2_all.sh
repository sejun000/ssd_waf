#!/bin/bash
# GS_FINAL: segment-granular ghost, same-block comparison.
#   F_push  = Σ valid_count at push time of D resident segs (per host-write).
#   F_ghost = Σ current valid_count of same segs (post-invalidate).
#   LHS = F_push - F_ghost (= invalidations within ghost = saved by extension).
#   RHS = Gud.  raise iff LHS > RHS.
# Usage: ./run_gs_SegG2_all.sh <R> [<MAW>]
set -u
...
run_one() {
    ...
    local SUFFIX=gsdec${RT}SegG2D${D}
