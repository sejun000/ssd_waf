#!/bin/bash
# Buggy reassign-form: ghost_sum = compacted_blocks + dt*rate at every compact event.
# decision tick = 1 seg fixed, util_step = D × seg/cache, RHS = Gud, hl = 4 seg.
# Usage: ./run_gs_dec1_bug_all.sh <R>
set -u
...
run_one() {
    ...
    local SUFFIX=gsdec${RT}dec1bugd${D}
...
echo "all 6 launched (R=$R, bug-form)"
