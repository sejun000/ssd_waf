#!/bin/bash
# GS_FINAL: ΔG = Gud/(4D) - Gu, ΔF = Fu - F_ud_age, raise iff ΔG < r·waf·ΔF.
#   Gu/Fu = running EWMA per host write (compaction_ratio / eviction_ratio).
#   F_ud_age = eviction_ratio_in_age_ghost_cache with log_cache_timestamp denom.
# Usage: ./run_gs_dGAg_all.sh <R> [<MAW>]
set -u
