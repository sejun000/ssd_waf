#!/bin/bash
# Clean GS_FINAL rule re-run on the 3 alibaba traces (ali 1/2/3) at r=8.64.
#   Rule: RAISE(GC) iff  Gud < r · invrate_sum
#   - Gud      = raw valid-page sum G(u+δ)         (no inv_rate correction to v)
#   - invrate_sum = Σ per-seg invalidate_rate over WT≤v band, NO victim subtraction
#   - GUD_GAP/TOPK/RESORT corrections removed from the binary entirely.
# Same GS_FINAL config as the resortN100 sweep (D=1, MAW=1 seg, r=8.64, SCALE=2)
# but with NO correction env vars (they no longer exist in the code).
set -u
...
run_one() {
    ...
    local TAG="GS_FINAL_clean_${TRACE_NAME}"
