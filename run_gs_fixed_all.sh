#!/bin/bash
# Re-run original GS (with monotone ghost_compacted_blocks_sum_ fix) for seg ∈ {1,2,4,8,16}.
# Usage: ./run_gs_fixed_all.sh <R>   e.g. 8.64 or 2.88
set -u
...
SCALE=2
R=${1:-8.64}
# RT is r without the dot (e.g. 8.64 -> 864, 2.88 -> 288)
RT=$(echo "$R" | tr -d '.')
SUFFIX=gsdec${RT}fix
...
run_one() {
    ...
    echo "[start segs=${P}] -> $STAT  log=$LOG  gsdec=$GSDEC"
    GS_DECISION_LOG="$GSDEC" ./cache_sim "$TRACE" "$CACHE" \
        ...
        > "$LOG" 2>&1 &
    echo "  pid=$!"
    sleep 1   # small stagger so timestamp suffix differs
}
...
echo "all 5 launched. running:"
ps -ef | grep cache_sim | grep "$POLICY" | grep -v grep | awk '{print $2,$NF}'
