#!/bin/bash
# GS_FINAL: same as SegG3 but F_pred/F_ghost/F_push update once per seg
# (was every seg/4). LHS = r·waf·(F_push - F_ghost), RHS = Gud.
# Usage: ./run_gs_SegG4_all.sh <R> [<MAW>]
set -u
...
run_one() {
    ...
    local SUFFIX=gsdec${RT}SegG4D${D}
