#!/bin/bash
# GS with decision=1seg hard-coded; gs_decision_period_segs controls util_step only.
# hl fixed at 4 segments = 6291456 blocks.
# Usage: ./run_gs_dec1_all.sh <R>
set -u
POLICY=LOG_GREEDY_COST_BENEFIT_10_GS
MA=ewma
MAW=6291456    # = 4 × segment_size_blocks (1572864)
...
run_one() {
    local D=$1     # delta_segs (= gs_decision_period_segs flag value, controls util_step)
    local TS=$(date +%y%m%d_%H%M%S)
    local SUFFIX=gsdec${RT}dec1hl4d${D}
    ...
    echo "[start delta=${D} r=${R} hl=${MAW}] gsdec=$GSDEC"
...
echo "all 6 launched (R=$R, hl=${MAW})"
