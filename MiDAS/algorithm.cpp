...
int group_split(struct SSD *ssd, struct STATS *stats, class GROUP *group[], int new_last){
    ...
    int old_total=0;
    int fill_size = static_cast<int>(group[new_last]->fill_queue.size());
    int mig_idx=0;
    int err_list[20] = {0,};
    int err_idx=0;
...
void active_split(struct SSD *ssd, struct STATS *stats, class GROUP *group[]){
    ...
    int ret;
    int gc_cnt = 0;
    while(fbqueue.size() < static_cast<std::size_t>(ssd_spec->FREENUM+(ssd_spec->NAIVE_N-1))){ //ensure the # of free segment for allocating the active segment for group of naive mida
...
void hot_merge(struct SSD* ssd, struct STATS* stats, class GROUP *group[]) {
    ...
    int G1=1;
    int move_seg;
    int hot_size = static_cast<int>(group[HOT]->fill_queue.size());
...
void merge_group(struct SSD *ssd, struct STATS *stats, class GROUP *group[]){
    ...
    active_merge(ssd, stats, group, target_g, victim_g);
    group[target_g]->size += group[victim_g]->size + 1; //+1: active segment

    //change the gnum info
    for(std::size_t i = 0; i < group[target_g]->fill_queue.size(); i++){
...
int gsize_check(struct SSD* ssd, class GROUP *group[]){
    ...
    for (int i = 0; i < ssd_spec->MAXGNUM; i++){
        ...
        if ((i < ssd_spec->naive_start) && (gsize[i] != static_cast<int>(group[i]->fill_queue.size()))) {
                abort();
            }
...
int checking_ginfo(struct SSD* ssd, struct STATS* stats, class GROUP *group[]) {
    ...
    if (APPLY_FLAG==1) {
        //naive mida off
        if (ssd_spec->mida_on==1) mida_on_off(ssd, stats, group, OFF);
        ginfo->commit_g=0;
        //merge or split
        change_config_2(ssd, stats, group);
