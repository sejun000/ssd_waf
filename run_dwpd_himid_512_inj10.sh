#!/bin/bash
# dwpd hi/mid: 640GB/5.12TiB config (the u90 geometry) but inject back to 10%.
#   cache 640GB, cold 5.12TB, segment 2GiB(=2^31, 298 segs), reserve 3.6TB, inject 10%.
#   reserve 3.6TB = "아까 정한대로" (640/5.12 util target). inject_frac 0.10 (was 0.20).
#   live = reserve 3.6 + WSS(hi 0.84/mid 1.03) = 4.44~4.63TB / 5.12 = 87~90% util.
...
RESERVE=3600000000000             # 3.6 TB synthetic reserve (아까 정한 양)
...
echo "launched 6 runs (cache640/cold5.12/seg2GiB/reserve3.6/inject10, cold->50TiB; hi/mid x REFLASH/FIFO/LRU)"
