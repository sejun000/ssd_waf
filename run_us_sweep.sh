#!/bin/bash
# Sweep r in {2,4,6,8,10} for one policy at a given util_step. Stat suffix tags us value.
# Usage: run_us_sweep.sh <POLICY> <UTIL_STEP>
set -u
POLICY="${1:-LOG_GREEDY_COST_BENEFIT_10}"
US="${2:-0.02}"
US_TAG=$(echo "$US" | sed 's/0\.//; s/\.//')   # 0.02 -> 02, 0.05 -> 05
...
TAG="${POLICY}_us${US_TAG}"

echo "Policy=${POLICY}  util_step=${US}  tag=${TAG}  cache=${CACHE}  ts=${TS}"
