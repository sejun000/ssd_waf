#!/usr/bin/env python3
"""Fixed WIN=64 (rows = segments) smoothing applied to all panels of the v4
EWMA r=8 sweep, plotting (pred-real)/real over time.
"""
import os, re
import numpy as np
import matplotlib
...
for ax, p in zip(axes.ravel(), PERIODS):
    ...
    ax.plot(t, err, color="#4C72B0", lw=1.2,
            label=f"(pred−real)/real  WIN={WIN} rows  WIN/δ={WIN/p:.2f}")
    ax.axhline(0.0, color="#DD8452", lw=0.9, ls="--", label="err=0")
    ax.set_ylim(-2, 2)
    row_dt = hw[1]-hw[0] if len(hw)>1 else 0
    ax.set_title(f"segs={p}  (δ={p} rows = {p*row_dt:.3f} TiB,  WIN/δ={WIN/p:.2f})", fontsize=11)
    ax.set_ylabel("(pred-real)/real", fontsize=10)
    ax.grid(alpha=0.3)
    ax.legend(loc="upper right", fontsize=8)
for ax in axes[-1]:
    ax.set_xlabel("host write t (TiB)", fontsize=10)
fig.suptitle(f"v4 EWMA r=8: smoothed-first relerr, WIN = {WIN} rows fixed across panels", fontsize=13)
fig.tight_layout()
for ext in ("pdf","png"):
    out = f"gs_v4_relerr_w{WIN}_r{R}.{ext}"
