...
#   bg seq inject, stop when COLD writes hit 40TiB (~8x cold capacity) so the cold
...
COLD_LIMIT=43980465111040          # 40 TiB cold writes (2x)
R=8.64
...
TAG=dwpd_hi_coldlim40_blk2g_REFLASH
echo "[launch] $TAG"
...
# --- LOG_FIFO baseline ---
TAG=dwpd_hi_coldlim40_blk2g_LOGFIFO
echo "[launch] $TAG"
stdbuf -oL ./cache_sim_blk2g "$TRACE" "$CACHE" $COMMON $INJECT \
