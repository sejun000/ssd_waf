...
def find_gs(p):
    # r=2.88: gsv4p1 for segs 1,2,4,8; gsv4 for segs 16.
    for tag in ("gsv4p1", "gsv4"):
        path = f"LOG_GREEDY_COST_BENEFIT_10_GS_us02_ewma_hl1572864_{tag}_segs{p}.stat_pr288"
...
plot_metric("gs_c", "cb_c",
            "Compacted blocks per WIN rows: GS r=2.88 vs CB_11 oracle@U(t)",
            "Δcompacted (GiB / 32 rows)",
            "gs_vs_cb_compacted_w32_288")
plot_metric("gs_f", "cb_f",
            "Evicted (flush) blocks per WIN rows: GS r=2.88 vs CB_11 oracle@U(t)",
            "Δevicted (GiB / 32 rows)",
            "gs_vs_cb_flushed_w32_288")
...
fig.tight_layout()
for ext in ("pdf",):
    out = f"A_gs_oracle_diff_and_tec_288.{ext}"
    fig.savefig(out, bbox_inches="tight"); print(f"wrote {out}")
plt.close(fig)
