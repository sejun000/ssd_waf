#!/bin/bash
# GS_FINAL: segment-granular age_ghost. Fghost = Σ valid_count over last D
# flushed segs (sampled per seg/4 tick, EWMA per host write).
#   LHS = Fpred - Fghost,  RHS = Gud,  raise iff LHS > RHS.
# Usage: ./run_gs_SegG_all.sh <R> [<MAW>]
set -u
