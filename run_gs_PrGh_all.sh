#!/bin/bash
# GS_FINAL: LHS = r·waf·(F_pred - F_ghost), RHS = Gud, raise iff LHS > RHS.
#   F_pred  = cum top-D valid pages from evict heap (snapshot per seg/4 tick).
#   F_ghost = cum valid pages in D last-flushed segs (snapshot per seg/4 tick).
# Usage: ./run_gs_PrGh_all.sh <R> [<MAW>]
set -u
