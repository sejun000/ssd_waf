#!/bin/bash
# GS_FINAL: cum_valid accumulator (tick), LHS = r·waf·δ, RHS = Gud (no Gu subtract).
# decision tick = 1 seg, util_step = D × seg/cache.
# Usage: ./run_gs_final_all.sh <R> [<MAW>]
#   MAW default = 6291456 (= 4 × seg). Use 1572864 for hl=1 seg.
set -u
...
run_one() {
    ...
    local SUFFIX=gsdec${RT}finald${D}
