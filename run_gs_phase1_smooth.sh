#!/bin/bash
# Phase 1 candidate tracker + SMA(64 segs) smoothed RMSE: segs 1~8, r=8.64.
set -u
...
R=8.64
R_TAG=864
TAG_SUFFIX=gsv4p1s   # p1 + smooth

cd "$(dirname "$0")"
TS=$(date +%y%m%d_%H%M%S)
echo "GS Phase-1 smooth sweep r=${R} hl=${MA_WINDOW} ts=${TS}"

for P in 1 2 4 8; do
    TAG="${POLICY}_us02_${MA_TYPE}_hl${MA_WINDOW}_${TAG_SUFFIX}_segs${P}"
