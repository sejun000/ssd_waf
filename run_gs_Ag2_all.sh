#!/bin/bash
# GS_FINAL: LHS = r·waf·(Fu - F_ud_age), RHS = Gud/(4D), raise iff LHS > RHS.
#   Fu = eviction_ratio.value() (log cache flush rate per host write).
#   F_ud_age = age_ghost valid evict rate (log_cache_timestamp denom).
#   Gud already marginal (ghost-sim of extra D segs/tick) → no Gu subtraction.
# Usage: ./run_gs_Ag2_all.sh <R> [<MAW>]
set -u
