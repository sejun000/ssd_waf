#!/bin/bash
# Sign-fixed marginal rule + ANTI-STUCK CAP: 29 consecutive RAISE -> force 1 flush
# (= 1 LOWER per 30, one-directional). tag = margcap30. Same alibaba setup.
...
launch() {
  ...
  local TAG=LOG_GREEDY_COST_BENEFIT_10_GS_FINAL_ewma_hl1572864_margcap30_gsD${d}_r${rtag}
...
wait
echo "ALL DONE $(date)" > margcap30_2r.DONE
