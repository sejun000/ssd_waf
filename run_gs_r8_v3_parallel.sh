#!/bin/bash
# GS v3 (segment/4 ratio update + segment/4 ghost-sum advance, decision at
# segment*segs).  r=8 sweep across all 8 segs in parallel.
set -u
...
echo "GS v3 r=${R} sweep, ts=${TS}, cache=${CACHE}"

for P in 1 2 4 8 16 32 64 128; do
    TAG="${POLICY}_us02_${MA_TYPE}_hl${MA_WINDOW}_gsv3_segs${P}"
    STAT="${TAG}.stat_pr${R}"
...
wait
echo "GS v3 r=${R} sweep done at $(date)"
