#!/bin/bash
# GS_FINAL: segment-granular ghost, same-block comparison, r·waf weighted.
#   F_push, F_ghost as in SegG2 (initial vs current valid of D resident segs).
#   LHS = r·waf·(F_push - F_ghost),  RHS = Gud,  raise iff LHS > RHS.
# Usage: ./run_gs_SegG3_all.sh <R> [<MAW>]
set -u
...
run_one() {
    ...
    local SUFFIX=gsdec${RT}SegG3D${D}
