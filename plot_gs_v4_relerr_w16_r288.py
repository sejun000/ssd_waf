#!/usr/bin/env python3
"""5-panel: relative error (pred-real)/real over time for v4 EWMA r=2.88 sweep.
Smoothing applied to pred (G_u_delta) and real (G_u) FIRST with WIN=16 rows,
then err computed on smoothed series.
"""
import os, re
import numpy as np
import matplotlib
...
def main():
    ...
    for ax, p in zip(axs, PERIODS):
        ...
        ax.plot(t, err, color="#4C72B0", lw=1.2, label="(pred-real)/real on smoothed series")
    ...
    fig.suptitle(f"v4 EWMA r=2.88: smoothed-first relative err (WIN={WIN} rows = {WIN} segments)", fontsize=13)
    fig.tight_layout()
    for ext in ("pdf","png"):
        out = f"gs_v4_relerr_w{WIN}_r{R_TAG}.{ext}"
