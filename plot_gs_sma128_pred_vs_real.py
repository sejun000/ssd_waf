#!/usr/bin/env python3
"""8-panel: G_t(u+δ) pred vs G_(t+δ)(u) realized for SMA-w128 v4 r=864.
WIN = 128 rows moving average applied to both pred and real before plotting.
"""
import os, re
import numpy as np
import matplotlib
...
R_TAG   = 864
TAG_FMT = "LOG_GREEDY_COST_BENEFIT_10_GS_us02_sma_w201326592_gsv4_segs{p}.stat_pr{r}"
...
def main():
    ...
    for ax, p in zip(axes.ravel(), PERIODS):
        ...
        ax.plot(t_axis, pred, color="#4C72B0", lw=1.4, label="G_t(u+δ)  pred")
        ax.plot(t_axis, real, color="#DD8452", lw=1.4, label="G_(t+δ)(u)  realized")
    ...
    fig.suptitle(f"SMA w=128segs: pred vs realized   r={R_TAG/100:.2f}, win={WIN} rows",
                 fontsize=13)
    fig.tight_layout()
    for ext in ("pdf", "png"):
        out = f"gs_sma128_pred_vs_real_r{R_TAG}.{ext}"
