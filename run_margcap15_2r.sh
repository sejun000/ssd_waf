#!/bin/bash
# Sign-fixed marginal rule + ANTI-STUCK CAP: 15 consecutive RAISE -> force 1 flush
# (15 RAISE : 1 LOWER, one-directional). tag = margcap15. Same alibaba setup.
...
launch() {
  ...
  local TAG=LOG_GREEDY_COST_BENEFIT_10_GS_FINAL_ewma_hl1572864_margcap15_gsD${d}_r${rtag}
...
wait
echo "ALL DONE $(date)" > margcap15_2r.DONE
