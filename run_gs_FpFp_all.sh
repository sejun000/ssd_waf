#!/bin/bash
# GS_FINAL: F_push/F_pop pop-method. age_ghost segment-granular FIFO holds last
# D flushed segs. F_push = cum valid pages ever pushed in. F_pop = cum valid
# pages aged out of ghost (= flushed even with D-seg extension).
#   LHS = r·waf·(F_push - F_pop), RHS = Gud, raise iff LHS > RHS.
# Usage: ./run_gs_FpFp_all.sh <R> [<MAW>]
set -u
