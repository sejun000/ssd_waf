#!/usr/bin/env python3
"""v4: G_t(u+δ) predicted vs G_{t+δ}(u) realized.  Same comparison as before
but using the v4 stat files (gsv4_segs*).  r=8.  segs ∈ {1,...,128} (8 panels).
"""
import os, re
import numpy as np
import matplotlib
    ...
    with open(path) as fh:
        for line in fh:
            ...
            try:
                ...
                comp.append(int(kv["compacted_blocks"]))
            except (KeyError, ValueError):
                continue
    return tuple(np.array(a) for a in (hw, gu, gud, comp))
...
def main():
    ...
    for ax, p in zip(axes.ravel(), PERIODS):
        ...
        ax.set_ylabel("GC rate", fontsize=10)
        ax.grid(alpha=0.3)
        ax.legend(loc="upper left", fontsize=8)
    for ax in axes[-1]:
        ax.set_xlabel("host write t (TiB)", fontsize=10)
    fig.suptitle(f"v4: ghost prediction accuracy  G_t(u+δ) vs G_{{t+δ}}(u)   r={R}",
                 fontsize=13)
    fig.tight_layout()
    for ext in ("pdf", "png"):
        out = f"gs_v4_g_prediction_vs_realized_r{R}.{ext}"
