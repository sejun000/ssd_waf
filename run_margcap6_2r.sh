#!/bin/bash
# Sign-fixed marginal rule + ANTI-STUCK CAP: 6 consecutive RAISE -> force 1 flush
# (one-directional). tag = margcap6. Same alibaba setup as margfix.
...
launch() {
  ...
  local TAG=LOG_GREEDY_COST_BENEFIT_10_GS_FINAL_ewma_hl1572864_margcap6_gsD${d}_r${rtag}
...
wait
echo "ALL DONE $(date)" > margcap6_2r.DONE
