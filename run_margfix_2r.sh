#!/bin/bash
# SIGN-FIXED marginal rule (RHS = r·waf·(Fnext−Fv) = r·waf·(FF−2F), was reversed).
# tag = margfix (keeps old buggy 'marg' results intact for comparison).
# d in {1,2,4,8,16} x r in {2.88, 8.64}. Same alibaba setup as marg/marg2.
cd /home/sejun000/ssd_waf
TRACE=/home/sejun000/alibaba_dwpd1to2_4x.trace
CACHE=1883510931456
LIMIT=15393162788864      # 14 TiB host write, single pass
CAP=10
MIN_FREE=300              # GB available required before launching another
declare -A RT=( [288]=2.88 [864]=8.64 )

launch() {
  local d=$1 rtag=$2 R=$3
  local TAG=LOG_GREEDY_COST_BENEFIT_10_GS_FINAL_ewma_hl1572864_margfix_gsD${d}_r${rtag}
...
wait
echo "ALL DONE $(date)" > margfix_2r.DONE
